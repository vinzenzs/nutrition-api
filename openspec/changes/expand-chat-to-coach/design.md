...
### D11 — Aggregate reads are dual-surface, preserving the drift-guard's subset invariant

The drift-guard (`mcpserver/drift_test.go`) asserts the chat surface is a **name-subset** of `AnnouncedToolNames`, minus a 4-entry `chatBespokeTools` allowlist that `toolnames.go` says "should only shrink." D10's aggregate reads (`get_training_context`, `get_recovery_context`, …) do not exist in mcpserver, so they would otherwise force that allowlist to *grow*. Decision: **each aggregate is mirrored into mcpserver**, not left chat-only. A new aggregate becomes (1) a REST `/context/*` endpoint, (2) an MCP tool registered in mcpserver + added to `AnnouncedToolNames`, and (3) an `agenttools` entry consumed by chat. The desktop coach gains the same aggregate (it wants `training_context` too), the subset invariant holds, and `chatBespokeTools` stays small / shrinks rather than absorbing every new aggregate. Net: aggregates are net-new endpoints worth their own small change ahead of phase 3 (as D10 already flagged), now explicitly dual-surface.

### D12 — One idempotency-key derivation, homed in `agenttools`

Two `deriveIdempotencyKey` impls exist and subtly disagree: `chat`'s decodes numbers as `float64` and strips nothing; `mcpserver`'s strips the `idempotency_key` field and uses `json.Number` (UseNumber), so `{"quantity_g":45.0}` hashes to different keys across the two. The key is a pure function of `(toolName, input)` — exactly what `agenttools` models — so lift the **richer** (mcpserver) derivation into `agenttools.DeriveIdempotencyKey` / `EffectiveIdempotencyKey` and have `chat` consume it now (mcpserver adopts it later, without needing the full tool port). Safe to change chat's derivation: the project is pre-release, derived keys only matter within a turn's replay window (none persisted across this change), and chat inputs never carry an `idempotency_key` field so stripping is a no-op for it; UseNumber is strictly higher-fidelity. This removes the latent divergence and means if cross-surface idempotency is ever wanted, the same input already yields the same key. Note: derivation is a *pure* helper — transport, auth, and the Idempotency-Key header still live in each consumer's dispatcher, so `agenttools` stays free of I/O.
