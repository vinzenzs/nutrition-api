...
The client SHALL parse the fields it cares about into typed values AND store the complete OFF response JSON unchanged for future re-extraction.

#### Scenario: Well-formed product parses all known nutriments

- **WHEN** the OFF response has `status: 1` and a `product.nutriments` object containing `energy-kcal_100g`, `proteins_100g`, `carbohydrates_100g`, `fat_100g`, `fiber_100g`, `sugars_100g`, `salt_100g`
- **THEN** the client extracts each value into the corresponding typed column on the product
...
- **WHEN** any individual nutriment field is absent from the OFF response
...
- **AND** the fixtures cover at minimum: a fully-populated product, a product with missing nutriments, a product with only kJ energy, a product with unparseable serving_size, and a `status: 0` not-found response
