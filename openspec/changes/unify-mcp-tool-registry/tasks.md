...
- [ ] 3.1 Port each domain: move typed arg structs into registry entries with `Build` funcs + tiers; delete bespoke `registerXxxTools` + handlers; migrate unit tests to Build-shape assertions. **Progress — 16/~28 domains, 48 tools:**
  ...
  - [ ] **MCP-only, remaining**: training-plan (13 hierarchical tools — likely its own batch)
