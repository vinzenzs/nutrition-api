# hydration-balance Specification

## Purpose

Store a daily water-balance snapshot — one row per calendar date — capturing a device's daily hydration estimate (initially Garmin's daily hydration response: `sweat_loss_ml`, `activity_intake_ml`, `goal_ml`). This is distinct from the `hydration` capability by grain and source: `hydration` records per-entry logged fluid intake (when and how much the user drank), whereas `hydration-balance` carries an external device's once-per-day estimate of sweat loss, activity-window intake, and the device's hydration goal. The two never merge — different grain (per-event vs per-day), different source (user-logged vs device-derived), different fields. The capability stays unit-isolated (millilitres only) with its own table, CRUD endpoints, and response shape, mirroring the parallel-table discipline used by `hydration`.
