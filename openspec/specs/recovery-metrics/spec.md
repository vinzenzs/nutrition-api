# recovery-metrics Specification

## Purpose

Define a persisted log of daily recovery/wellness snapshots — sleep, HRV, resting heart rate, stress, body battery, and training readiness — captured one row per calendar date, independent of every other capability. The shape is source-agnostic: any writer (initially `garmin.py`, later any wellness source) targets it via the REST endpoints, "POSTing every day it sees" without tracking sync state. NULL on any metric is a meaningful state ("the device did not report it for that day"), not a data-quality bug. Recovery metrics are kept in their own response shape and never merged into nutrition, hydration, or fitness totals — sleep is seconds, HRV is milliseconds, and the remaining signals are unitless 0–100 scores.
## Requirements
