...
Capture logged fluid-intake events as first-class, unit-isolated primitives the agent can reason over without polluting the nutrition aggregation surface. Hydration is structurally different from food — volume not mass, no nutriments, no recipes, no Open Food Facts lookup — so it lives in a parallel `hydration_entries` table with its own CRUD endpoints and its own daily summary, mirroring the meals shape but keeping units cleanly separate (ml only, never mixed with grams). The capability provides the primitive (when and how much was drunk, with optional free-text context in `note`); higher-level reasoning about beverage type, workout-window tagging, sweat rate, and adherence to context-dependent targets stays in the agent layer, which has access to the user's training data. This deliberately scopes the API to recording rather than prescribing: no beverage enum, no stored hydration goal, no integration with `/summary/daily` (the nutrition one), no caffeine/alcohol tracking — those are explicit non-goals captured in the proposal.
