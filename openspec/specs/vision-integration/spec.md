# vision-integration Specification

## Purpose

Define how the server integrates with Anthropic's Claude Vision API to parse meal photos into structured nutriment estimates. Image bytes never persist; tool-forced output keeps the contract narrow; one configurable env var (`ANTHROPIC_API_KEY`) controls whether the capability is on. The `meals` capability consumes this via `POST /meals/from_photo`.
