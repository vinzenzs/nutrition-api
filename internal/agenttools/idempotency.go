...
// caller did not supply one explicitly. It hashes the tool name plus the
// canonical JSON of args, with the idempotency_key field stripped if present
// (otherwise the key would depend on itself). args may be a struct or a
// json.RawMessage — both round-trip through encoding/json.
//
// This is the single shared implementation consumed by both internal/chat's
// loopback dispatcher and internal/mcpserver (D12), so a write replayed from
// either surface hashes identically and hits the same REST idempotency record.
...
// derived key be stable across reorderings of struct fields and across the
// presence/absence of an explicit idempotency_key.
