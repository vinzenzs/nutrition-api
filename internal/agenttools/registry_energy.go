...
// Energy Availability read — computes EA over a date window from intake,
// exercise burn, and body composition. Ported from internal/mcpserver onto the
// shared registry (unify-mcp-tool-registry). MCP-only read; the description and
// arg struct are byte-identical to the prior bespoke registration so the
// announced schema is unchanged.

func init() { registerMCPDomain(energySpecs()) }
	...
	return []Spec{
		{
			...
			SchemaType: WeeklyEnergySummaryArgs{},
			Tier:       TierRead,
			Build: func(in json.RawMessage) (HTTPCall, error) {
				var a WeeklyEnergySummaryArgs
				...
				if a.LeanMassKg != nil {
					q.Set("lean_mass_kg", strconv.FormatFloat(*a.LeanMassKg, 'f', -1, 64))
				}
				if a.BodyFatPct != nil {
					q.Set("body_fat_pct", strconv.FormatFloat(*a.BodyFatPct, 'f', -1, 64))
				}
				return HTTPCall{Method: "GET", Path: "/energy/availability", Query: q}, nil
			},
		},
	}
}
